...
export function TableCellRenderer({ 
  ...
  switch (column.render?.type) {
      ...
      return (
        <div className={"userCell"}>
          ...
          {column.render.showSubtext && subtextValue && (
            <div className={"userMeta"}>{subtextValue}</div>
          )}
        </div>
      );
    ...
    case 'email':
      return (
        <span className={`${"emailColumn"} ${column.render.className || ''}`}>
          {value}
        </span>
      );
    ...
    case 'badge':
      ...
      if (column.render?.colorMap) {
        ...
        return (
          <span className={`${"badge"} ${`${colorClass}Badge`}`}>
            {value}
          </span>
        );
      }
    ...
    case 'role_badge':
      if (!value) {
        return <span className={"noRoles"}>{column.render.nullText || '-'}</span>;
      }
      return (
        <div className={"rolesList"}>
          <span className={`${"roleBadge"} ${`role-${value}`}`}>
            {value}
          </span>
        </div>
      );
    ...
    case 'multi_role_badges':
      if (!value || !Array.isArray(value) || value.length === 0) {
        return <span className={"noRoles"}>{column.render?.emptyText || '-'}</span>;
      }
      ...
      return (
        <div className={`${"rolesList"} ${column.render?.className || ''}`}>
          {visibleRoles.map((role, index) => (
            <span key={`${role}-${index}`} className={`${"roleBadge"} ${`role-${role}`}`}>
              {role}
            </span>
          ))}
          {remainingCount > 0 && column.render?.showCount && (
            <span className={"moreRoles"}>
              +{remainingCount}
            </span>
          )}
        </div>
    ...
    case 'count':
      return (
        <span className={`${"characterCount"} ${column.render.className || ''}`}>
    ...
    case 'status_badge':
      ...
      return (
        <span className={`${"statusBadge"} ${statusConfig.className || ''}`}>
    ...
    case 'priority_badge':
      ...
      return (
        <span className={`${"priorityBadge"} ${priorityConfig.className || ''}`}>
    ...
    case 'account_status_badge':
      ...
      return (
        <span 
          className={`${"statusBadge"} ${matchedStatus.className || ''} ${isClickable ? "clickable" : ''}`}
    ...
    default:
      ...
      if (!value && column.render?.emptyText) {
        return <span className={"emptyText"}>{column.render.emptyText}</span>;
      }
