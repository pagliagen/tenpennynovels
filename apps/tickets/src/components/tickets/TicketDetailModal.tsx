...
export const TicketDetailModal: React.FC<TicketDetailModalProps> = ({
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      OPEN: { label: 'Aperto', className: "statusOpen" },
      IN_PROGRESS: { label: 'In Corso', className: "statusInProgress" },
      WAITING_FOR_USER: { label: 'In Attesa', className: "statusWaiting" },
      ESCALATED: { label: 'Escalato', className: "statusEscalated" },
      CLOSED: { label: 'Chiuso', className: "statusClosed" }
    };
  ...
  const getPriorityBadge = (priority: string) => {
    const priorityConfig = {
      low: { label: 'Bassa', className: "priorityLow" },
      medium: { label: 'Media', className: "priorityMedium" },
      high: { label: 'Alta', className: "priorityHigh" },
      urgent: { label: 'Urgente', className: "priorityUrgent" },
      critical: { label: 'Critica', className: "priorityCritical" }
    };
    
    const config = priorityConfig[priority as keyof typeof priorityConfig] || priorityConfig.low;
    
    return (
      <span className={`${"priorityBadge"} ${config.className}`}>
        {config.label}
      </span>
    );
  };
  ...
  const renderDynamicFooter = () => {
    ...
    switch (activeAction) {
      case 'assign':
        return (
          <div className={"dynamicActions"}>
            <div className={"leftSection"}>
              <span className={"actionLabel"}>Assegna a:</span>
              <select
      ...
      case 'change_status':
        ...
        return (
          <div className={"dynamicActions"}>
            <div className={"leftSection"}>
              <span className={"actionLabel"}>Cambia stato a:</span>
              <select
      ...
      case 'transfer':
        ...
        return (
          <div className={"dynamicActions"}>
            <div className={"leftSection"}>
              <span className={"actionLabel"}>Trasferisci a:</span>
              <select
      ...
      case 'close':
        return (
          <div className={"dynamicActions"}>
            <div className={"leftSection"}>
              <span className={"actionLabel"}>Motivo chiusura:</span>
              <input
                type="text"
        ...
        <div className={"body"}>
          ...
          {currentError && (
            <div className={"errorState"}>
              <p>⚠️ {currentError}</p>
              <button onClick={loadTicketData} className={"retryButton"}>
                Ricarica Dati
              </button>
            </div>
          )}
          ...
          {!currentLoading && !currentError && ticket && (
            <div className={"content"}>
              ...
              <div className={"section"}>
                <h3 className={"sectionTitle"}>Informazioni Ticket</h3>
                <div className={"infoGrid"}>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>ID:</span>
                    <span className={"infoValue"}>#{ticket.id}</span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Categoria:</span>
                    <span className={"infoValue"}>{ticket.categoryLabel}</span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Stato:</span>
                    <span className={"infoValue"}>{getStatusBadge(ticket.status)}</span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Priorità:</span>
                    <span className={"infoValue"}>{getPriorityBadge(ticket.priority)}</span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Creato da:</span>
                    <span className={"infoValue"}>{ticket.characterName}</span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Reparto:</span>
                    <span className={"infoValue"}>{getDepartmentLabel(ticket.department)}</span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Assegnato a:</span>
                    <span className={"infoValue"}>
                      {ticket.assignedStaff ? ticket.assignedStaff.name : 'Non assegnato'}
                    </span>
                  </div>
                  <div className={"infoItem"}>
                    <span className={"infoLabel"}>Creato il:</span>
                    <span className={"infoValue"}>{formatDate(ticket.createdAt)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className={"section"}>
                ...
                <div className={"messagesContainer"}>
                  {ticket.messages?.map((message) => (
                    <div
                      key={message.id}
                      className={`${"message"} ${
                        message.isFromStaff ? "messageStaff" : "messageUser"
                      } ${message.isInternal ? "messageInternal" : ''}`}
                    >
                      <div className={"messageHeader"}>
                        <span className={"messageAuthor"}>
                          {message.authorName}
                          {message.authorRoles && message.authorRoles.length > 0 && (
                            <span className={"authorRoles"}>
                        ...
                        {message.isInternal && (
                          <span className={"internalBadge"}>Interno</span>
                        )}
                      </div>
                ...
                <form onSubmit={handleSendMessage} className={"messageForm"}>
                    <div className={"messageInputGroup"}>
                      <textarea
                      ...
                      <div className={"messageOptions"}>
                        ...
                        {!ticket?.assignedStaff && !messageForm.isInternal && (
                          <div className={"autoAssignNotice"}>
