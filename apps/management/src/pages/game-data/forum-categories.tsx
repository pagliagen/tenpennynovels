...
} from '@/hooks/api/useForumCategories';
...
import type { TopicAccessRule, AccessRuleType } from '@/types/api/ForumTopic';
import { ACCESS_RULE_TYPE_LABELS, GAMEPLAY_ROLE_OPTIONS } from '@/types/api/ForumTopic';
import styles from '@/styles/pages/ForumCategories.module.scss';
...
const EMPTY_FORM: CreateForumCategoryData = {
  title: '',
  description: '',
  sortOrder: 0,
  defaultAccessRules: [],
...
export default function ForumCategoriesPage() {
  const { filters, params, setParams, handleFilterChange } = useTableFilters<ForumCategoryListParams>({
    page: 1, pageSize: 25, sortBy: 'sortOrder', sortOrder: 'asc',
  });
  const [modalOpen, setModalOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<ForumCategory | null>(null);
  const [formData, setFormData] = useState<CreateForumCategoryData>(EMPTY_FORM);
  ...
  const { data, isLoading, error } = useForumCategories(params);
  const tableConfig = useTableConfig('forum-categories');
  ...
  const openEditModal = useCallback((category: ForumCategory) => {
  ...
  const handleAddRule = useCallback(() => {
    ...
    setFormData(prev => ({
      ...prev,
      defaultAccessRules: [...(prev.defaultAccessRules || []), rule],
    }));
    setNewRuleType('public');
    setNewRuleCorporationId('');
    setNewRuleGameplayRole('');
  ...
  const handleRemoveRule = useCallback((index: number) => {
    setFormData(prev => ({
      ...prev,
      defaultAccessRules: (prev.defaultAccessRules || []).filter((_, i) => i !== index),
    }));
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        const updateData: UpdateForumCategoryData = { ...formData };
        await updateCategory.mutateAsync({ categoryId: editingCategory._id, data: updateData });
        addNotification({ type: 'success', message: `Categoria "${formData.title}" aggiornata` });
      } else {
        await createCategory.mutateAsync(formData);
        addNotification({ type: 'success', message: `Categoria "${formData.title}" creata` });
      }
      closeModal();
    } catch (err) {
  ...
  const handleAction = async (action: string, category: ForumCategory) => {
    try {
      if (action === 'edit') {
        ...
        const confirmed = await confirm({
          ...
          message: `Sei sicuro di voler eliminare la categoria "${category.title}"? Gli argomenti associati resteranno non categorizzati.`,
        });
        if (confirmed) {
          await deleteCategory.mutateAsync({ categoryId: category._id });
          addNotification({ type: 'success', message: `Categoria "${category.title}" eliminata` });
        }
      }
    } catch (err) {
    ...
    return (
      <ManagementLayout>
        <div className={styles.errorContainer}>
          <h2>Errore nel caricamento categorie</h2>
  ...
  return (
    <ManagementLayout>
      <Head><title>Ten Penny Novels | Forum - Categorie</title></Head>
      <div className={styles.forumCategories}>
        <header className={styles.header}>
          <div>
            <h1>Forum - Categorie</h1>
            <p>Totale: {totalItems} categorie</p>
          </div>
          <div className={styles.headerActions}>
            <button className={styles.createButton} onClick={openCreateModal}>+ Nuova Categoria</button>
          </div>
        </header>

        <ConfigurableDataTable<ForumCategory>
          tableName="forum-categories"
          data={categories}
        ...
        <Modal
          isOpen={modalOpen}
          onClose={closeModal}
          title={editingCategory ? `Modifica: ${editingCategory.title}` : 'Nuova Categoria'}
          size="large"
          footer={
            <div className={styles.modalFooter}>
              ...
              >
                {isSaving ? 'Salvataggio...' : editingCategory ? 'Salva Modifiche' : 'Crea Categoria'}
              </button>
            </div>
          }
        >
          <form onSubmit={handleSubmit} className={styles.modalForm}>
            ...
            <p className={styles.helpText}>
              Definisce chi può visualizzare le bacheche di questa categoria per default.
              Ogni bacheca può comunque avere regole proprie che sovrascrivono queste
              (impostando "Regole personalizzate" sulla bacheca). Se nessuna regola è
              definita, la categoria è accessibile a tutti.
            </p>

            <div className={styles.accessRulesSection}>
              {(formData.defaultAccessRules || []).length > 0 && (
                <div className={styles.accessRulesList}>
                  {(formData.defaultAccessRules || []).map((rule, index) => (
