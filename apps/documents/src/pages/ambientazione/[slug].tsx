...
export default function AmbientazioneDocument() {
  ...
  useEffect(() => {
    async function loadAuthData() {
      try {
        // Try to fetch any document to detect auth - simple approach
  ...
  useEffect(() => {
    ...
    async function checkFavoriteStatus() {
      try {
        const favorited = await isDocumentFavorited('ambientazione', slug as string);
    ...
    async function fetchDocumentAndList() {
      try {
        ...
        const [content, allDocs] = await Promise.all([
          getDocument('ambientazione', slug as string),
          getDocuments('ambientazione')
        ]);
  ...
  const handleFavoriteToggle = async () => {
    ...
    try {
      ...
      if (isFavorited) {
        await removeDocumentFromFavorites('ambientazione', slug as string);
        setIsFavorited(false);
      } else {
        await addDocumentToFavorites('ambientazione', slug as string);
        setIsFavorited(true);
      }
    } catch (error) {
      ...
      <DocumentsLayout authContext={authContext}>
        ...
        <div className={styles.documentContainer}>
          <div className={styles.errorState}>
            ...
            <Link href="/ambientazione" className={styles.backButton}>
              ← Torna all'elenco ambientazione
            </Link>
          </div>
        </div>
      </DocumentsLayout>
  ...
  return (
    <DocumentsLayout authContext={authContext}>
      <Head>
        <title>{document.title} - TenpennyNovels Ambientazione</title>
        <meta
          name="description"
          content={document.description || `Documento di ambientazione: ${document.title}`}
        />
      </Head>
      ...
      <div className={styles.documentContainer}>
        ...
        <header className={styles.documentHeader}>
          <nav className={styles.breadcrumb}>
            ...
            <Link href="/ambientazione">Ambientazione</Link>
            <span className={styles.separator}>›</span>
          ...
          <div className={styles.navigation}>
            {(() => {
              ...
              return (
                <>
                  {prevDoc ? (
                    <Link href={`/ambientazione/${prevDoc.slug}`} className={styles.backButton}>
                      ← {prevDoc.title}
                    </Link>
                  ) : (
                  ...
                  {nextDoc ? (
                    <Link href={`/ambientazione/${nextDoc.slug}`} className={styles.nextButton}>
                      {nextDoc.title} →
                    </Link>
                  ) : (
