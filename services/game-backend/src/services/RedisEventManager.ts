...
export class RedisEventManager {
  ...
  private async handleCharacterReview(message: string): Promise<void> {
    try {
      ...
      await this.sendCharacterReviewMessage(characterId, characterName, action, note, reviewedByUsername, adminCookies);
  ...
  private async sendCharacterReviewMessage(
    ...
    try {
      ...
      // Use the API endpoints to send the message
      await this.sendOffGameMessageViaAPI(
        adminCharacter._id.toString(),
        targetCharacter._id.toString(),
        messageContent
      );

      logger.info(`Character review message sent via API`, {
  ...
  /**
   * Send off-game message using HTTP calls to game backend  
   */
  private async sendOffGameMessageViaAPI(
    fromCharacterId: string,
    toCharacterId: string,
    messageContent: string
  ): Promise<void> {
    try {
      const GAME_BACKEND_URL = process.env.GAME_BACKEND_URL || 'http://localhost:3001';
      
      // 1. Create or get existing direct chat
      const chatResponse = await fetch(`${GAME_BACKEND_URL}/game/offgame-chats`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Use same cookies as frontend would use - admin character session
          'Cookie': `auth_token=${process.env.ADMIN_AUTH_TOKEN}; character_context=${process.env.ADMIN_CHARACTER_TOKEN}`
        },
        body: JSON.stringify({
          type: 'direct',
          participants: [toCharacterId]
        })
      });
      ...
      const chatId = chatResult.data._id;
      ...
      // 2. Send the message
      const messageResponse = await fetch(`${GAME_BACKEND_URL}/game/offgame-chats/${chatId}/messages`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Use same cookies as frontend would use - admin character session
      ...
      logger.info('Review message sent successfully via HTTP', {
        chatId,
      ...
      logger.error('Error in sendOffGameMessageViaAPI:', {
        error: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined,
        fromCharacterId,
        toCharacterId
  ...
  private async handleCharacterMoved(event: any): Promise<void> {
    ...
    if (event.newLocationId) {
      ...
      // Broadcast global presence updates for all clients
      if (event.oldLocationId) {
        console.log(`🌍 RedisEventManager: Broadcasting global presence update - character left location`);
      ...
      console.log(`🌍 RedisEventManager: Broadcasting global presence update - character entered location`);
      this.io.emit('global_presence_update', {
