...
// Fase 7.4: pre('validate'), non pre('save') — retentionUntil è required,
// e Mongoose esegue la validazione dei campi required PRIMA di pre('save')
// (solo pre('validate') gira prima). Con pre('save') la validazione falliva
// sempre: MessageBackupService.createBackup() non ha mai funzionato, per
// nessuno dei due messageContext. Bug preesistente corretto su decisione
// esplicita dell'utente, scoperto dal test E2E di questa fase.
MessageBackupSchema.pre('validate', function(this: IMessageBackup) {
