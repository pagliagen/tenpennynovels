...
/** Fase 7.2: doc appena passati a playerStatus 'pending' in pre('save'), letti e ripuliti in post('save') — stesso pattern di features/forum/models/ForumPost.ts. */
const pendingApprovalDocs = new WeakSet<Document>();
...
// Fase 7.2: emette l'extension point per la richiesta di approvazione dopo
// che il personaggio è salvato per davvero — vedi pendingApprovalDocs sopra.
CharacterSchema.post('save', async function(doc) {
  if (!pendingApprovalDocs.has(doc)) return;
  pendingApprovalDocs.delete(doc);

  await extensions.emit('character.playerStatus.pending', {
    characterId: doc._id.toString(),
    characterName: doc.name,
    characterAvatar: doc.avatar,
  });
});
