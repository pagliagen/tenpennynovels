...
const server = app.listen(PORT, async () => {
  ...
  try {
    ...
    const { setSocketIO } = await import('./websocket/socketInstance');
    ...
    // Attach io instance to app for use in controllers (legacy)
    app.set('io', io);

    // Register io instance in singleton (preferred method)
    setSocketIO(io);
