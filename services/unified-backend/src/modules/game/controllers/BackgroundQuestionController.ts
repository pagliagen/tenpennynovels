...
export class BackgroundQuestionController {
   ...
   */
  static async getQuestionsByCategory(req: Request<{ category: string }>, res: Response): Promise<void> {
    try {
   ...
   */
  static async getCharacterBackgroundResponses(req: Request<{ characterId: string }>, res: Response): Promise<void> {
    try {
   ...
   */
  static async updateBackgroundResponses(req: Request<{ characterId: string }>, res: Response): Promise<void> {
    try {
      const { characterId } = req.params;
      ...
      for (const responseData of responses as any[]) {
        ...
        if ((question as any).minLength && response.trim().length < (question as any).minLength) {
          res.status(400).json(errorResponse(
            `Risposta troppo breve per la domanda ${questionId}`,
            'RESPONSE_TOO_SHORT',
            { questionId, minLength: (question as any).minLength },
            400,
            getRequestId(req)
          ));
          return;
        ...
        if (response.trim().length > (question as any).maxLength) {
          res.status(400).json(errorResponse(
            `Risposta troppo lunga per la domanda ${questionId}`,
            'RESPONSE_TOO_LONG',
            { questionId, maxLength: (question as any).maxLength },
            400,
            getRequestId(req)
          ));
          return;
        ...
        // Salva la risposta
        character.setBackgroundResponse(questionId, response, (question as any).version);
      }
