...
export interface IForumDiscussion extends Document {
  ...
  isVisible: boolean;
  postCount: number;
  subscriberCount: number; // NEW: Denormalized count for performance
  viewCount: number;
  lastPostAt?: Date;
  lastPostBy?: {
    userId: string;
...
export const ForumDiscussion = mongoose.model<IForumDiscussion>('ForumDiscussion', ForumDiscussionSchema);
