...
export class ForumFollowController {
  static async follow(req: Request<{ characterId: string }>, res: Response): Promise<void> {
    try {
      ...
      if (followerId.equals(followedId)) {
        res.status(400).json(errorResponse('Cannot follow yourself', 'CANNOT_FOLLOW_SELF', undefined, 400, getRequestId(req)));
        return;
      }
      ...
      if (existing) {
        ...
        res.status(200).json(successResponse({
          following: false
        }, 'Unfollowed character', getRequestId(req)));
      } else {
        ...
        res.status(201).json(createResponse({
          following: true
        }, 'Now following character', getRequestId(req)));
      }
    } catch (error: unknown) {
      console.error('[ForumFollowController] Follow error:', error);
      res.status(500).json(errorResponse('Failed to toggle follow', 'FOLLOW_ERROR', undefined, 500, getRequestId(req)));
    }
  }
  ...
  static async getMyFollows(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json(errorResponse('Failed to fetch follows', 'GET_MY_FOLLOWS_ERROR', undefined, 500, getRequestId(req)));
    }
  }
  ...
  static async getFollowing(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json(errorResponse('Failed to fetch following', 'GET_FOLLOWING_ERROR', undefined, 500, getRequestId(req)));
    }
  }
  ...
  static async getFollowers(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json(errorResponse('Failed to fetch followers', 'GET_FOLLOWERS_ERROR', undefined, 500, getRequestId(req)));
    }
  }
}
