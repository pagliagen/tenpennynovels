...
import { parseChunks } from '../services/ChunkParser';
...
export class DocumentManagementController {
  ...
  /**
   * Get paginated list of routes (NEW ARCHITECTURE)
   * Uses Route model (routing layer) + Document model (content layer)
   * GET /admin/documents?page=1&pageSize=25&sortBy=createdAt&sortOrder=desc&type=ambientazione|regolamento
   */
  static async getDocuments(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Build sort (map old field names to new schema)
      let sortField = sortBy as string;
      if (sortField === 'metadata.createdAt') sortField = 'createdAt';
      if (sortField === 'metadata.updatedAt') sortField = 'updatedAt';
      const sortObj: any = { [sortField]: sortOrder === 'desc' ? -1 : 1 };

      // Query routes with populated document
      const [routes, totalItems] = await Promise.all([
        Route.find(filter)
          .sort(sortObj)
          .skip(skip)
          .limit(limitNum)
          .populate('rootDocumentId', 'slug title contentDelta isDraft tags')
          .lean(),
      ...
      res.json({
        result: true,
        success: true,
        list: routes.map((route: any) => {
          const doc = route.rootDocumentId;

          return {
            _id: route._id.toString(),
            title: route.title,
            slug: doc?.slug || route.path,
            type: route.type,
            path: route.path,
            kind: route.kind,
            displayCategory: route.displayCategory,
            isPublished: route.isPublic && route.enabled,
            enabled: route.enabled,
            isPublic: route.isPublic,
            isDraft: doc?.isDraft || false,
            order: 0, // Route model has no order field
            group: route.displayCategory ? { name: route.displayCategory } : null,
            metadata: {
   ...
   */
  static async updateDocumentGroup(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const { id } = req.params;
   ...
   */
  static async deleteDocumentGroup(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const { id } = req.params;
   ...
   */
  static async reorderDocuments(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const { id } = req.params;
  ...
  static async createDocument(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Parse chunks (H2/H3 sections) from content and create DocumentChunks
      try {
        // Assume content is TipTap Delta JSON (from frontend editor)
        let contentDelta: any;
        try {
          // Try parsing as JSON first
          contentDelta = typeof content === 'string' ? JSON.parse(content) : content;
        } catch (e) {
          // Fallback: if content is plain text/HTML, create single chunk
          contentDelta = {
            type: 'doc',
            content: [{
              type: 'paragraph',
        ...
        const parsedChunks = parseChunks(contentDelta);
        logger.info(`Parsed ${parsedChunks.length} chunks from document: ${title}`);

        // Create DocumentChunks in database
        ...
        for (const chunk of parsedChunks) {
          const chunkData = {
            ...
            content: chunk.content, // Plain text for embeddings
            order: chunk.order,
            documentType: type,
            version: 1,
            isActive: true,
            contentEmbedding: undefined, // Will be set by worker
          ...
          // Publish Redis event for chunk embedding generation
          try {
            await embeddingPublisher.publishDocumentChunkEvent(
            ...
            logger.info(`Published chunk embedding event: ${chunk.heading} (${chunk.slug})`);
          } catch (chunkError) {
            logger.error(`Failed to publish chunk embedding event for ${chunk.heading}:`, chunkError);
          }
        }

        logger.info(`Created ${parsedChunks.length} DocumentChunks for document: ${title}`);
      } catch (chunkError) {
        // Don't fail the request if chunk parsing fails
        logger.error('Failed to parse/create document chunks:', chunkError);
      }

      // Regenerate sitemap for SEO (if document is public)
      try {
        const sitemapService = new SitemapService();
        await sitemapService.generateSitemap();
        logger.info('[DocumentManagement] Sitemap regenerated after document creation');
      } catch (error) {
        // Don't fail the request if sitemap generation fails
        logger.error('[DocumentManagement] Failed to regenerate sitemap:', error);
      }
   ...
   */
  static async updateDocument(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      if (content) {
        ...
        // Update DocumentChunks: deactivate old chunks and create new ones
        try {
          const db = mongoose.connection.db!;
          ...
          logger.info(`Deactivated old chunks for document: ${id}`);
          ...
          const parsedChunks = parseChunks(contentDelta);
          logger.info(`Parsed ${parsedChunks.length} chunks for document update: ${id}`);

          // Get max version from old chunks
          const oldChunks = await db.collection('documentchunks')
            .find({ documentId: docObjectId })
            .sort({ version: -1 })
            .limit(1)
            .toArray();
          const newChunkVersion = (oldChunks[0]?.version || 0) + 1;
          ...
          for (const chunk of parsedChunks) {
            ...
            // Publish Redis event for chunk embedding
            try {
              await embeddingPublisher.publishDocumentChunkEvent(
                chunkResult.insertedId.toString(),
                id,
                ...
                chunk.heading,
                chunk.content,
                existingDoc!.type as 'ambientazione' | 'regolamento',
                chunk.order
              );
              logger.info(`Published chunk update event: ${chunk.heading} (${chunk.slug})`);
            } catch (chunkError) {
              logger.error(`Failed to publish chunk update event for ${chunk.heading}:`, chunkError);
            }
          }

          logger.info(`Created ${parsedChunks.length} new DocumentChunks (v${newChunkVersion}) for document: ${id}`);
        } catch (chunkError) {
          // Don't fail the request if chunk update fails
          logger.error('Failed to update document chunks:', chunkError);
        }
      }
      ...
      // Regenerate sitemap for SEO (if document visibility changed to/from public)
      try {
        const sitemapService = new SitemapService();
        await sitemapService.generateSitemap();
        logger.info('[DocumentManagement] Sitemap regenerated after document update');
      } catch (error) {
        // Don't fail the request if sitemap generation fails
        logger.error('[DocumentManagement] Failed to regenerate sitemap:', error);
      }
   ...
   */
  static async deleteDocument(req: Request<{ id: string }>, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      // Regenerate sitemap for SEO (if deleted document was public)
      try {
        const sitemapService = new SitemapService();
        await sitemapService.generateSitemap();
        logger.info('[DocumentManagement] Sitemap regenerated after document deletion');
      } catch (error) {
        // Don't fail the request if sitemap generation fails
        logger.error('[DocumentManagement] Failed to regenerate sitemap:', error);
      }
