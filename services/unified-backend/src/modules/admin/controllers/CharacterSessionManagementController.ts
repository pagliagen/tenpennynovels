...
export class CharacterSessionManagementController {
  ...
  static async getCharacterSessionHistory(req: Request<{ characterId: string }>, res: Response): Promise<void> {
    try {
  ...
  static async invalidateAllCharacterSessions(req: Request<{ characterId: string }>, res: Response): Promise<void> {
    try {
      const characterId = req.params.characterId;
      const { reason } = req.body;
