...
export class OccupationManagementController {
   ...
   *
   * ATTENZIONE — bug preesistente, preservato esattamente: la query sotto usa
   * 'occupation.occupationId' come path su Character, ma lo schema reale ha
   * `character.occupation` come ObjectId diretto (senza subfield `occupationId`,
   * vedi CharacterGameplayController.ts: `character.occupation = occupation._id`).
   * La query non trova quindi mai nulla — "disattiva invece di eliminare se in
   * uso" non scatta mai correttamente, si va sempre al ramo soft-delete.
   */
