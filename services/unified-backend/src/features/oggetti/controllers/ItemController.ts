    ...
    try {
      ...
      const itemsWithEligibility = await Promise.all(items.map(async (item) => {
        ...
        return {
          ...
          // Bug preesistente preservato: letti come campi di primo livello, ma lo
          // schema li annida sotto item.properties (e maxStack è properties.maxQuantity
          // sullo schema, nome diverso) — sempre undefined, fallback costante.
          isStackable: (item as any).isStackable || false,
          maxStack: (item as any).maxStack || 1,
  ...
  static async getItemDetails(req: Request<{ itemId: string }>, res: Response): Promise<void> {
    try {
      ...
      const itemDetails = {
        ...
        // Item properties — bug preesistente preservato, vedi getAvailableItems
        isStackable: (item as any).isStackable || false,
        maxStack: (item as any).maxStack || 1,
...
// NOTA: controlla solo stats/skills/gender/age — diverge da ShopController.meetsRequirements
// (ex EconomyController), che controlla anche occupations/corporations/social-class/
// financial-classes. Due implementazioni distinte dello stesso concetto, non unificate
// in questa fase (pre-esistente).
