...
export const clientRateLimit = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: (req: Request) => (req as any).client?.id || req.ip || 'unknown',
  max: (req: Request) => (req as any).client?.rateLimit?.maxPerMinute || DEFAULT_MAX_PER_MINUTE,
  message: (_req: Request, res: Response) => {
    const client = (res.req as any).client;
    return {
      success: false,
