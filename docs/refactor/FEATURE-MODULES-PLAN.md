...
**7.4 — Chat + ChatBackup + OnGame + MessageService split: fatta (2026-08-14), ultima sotto-fase.** La ricognizione ha ridimensionato quella che il piano originale descriveva come "la più grande/rischiosa": **nessuno dei 5 model coinvolti (`Chat`, `ChatBackup`, `OnGameMessage`, `OnGameThread`, `MessageBackup`) aveva un import di una feature.** Gli unici accoppiamenti reali trovati (import di `@features/fineSessione/api`/`@features/documenti/api` in `ChatController.ts`, di `@features/offGameMessages/api` in `websocket/chatHandlers.ts`) stanno in file che, su decisione esplicita dell'utente, **non sono entrati nel perimetro**: restano in `modules/game/` come `CharacterController.ts`/`LocationController.ts` in 7.2/7.3, liberi di importare feature via `api.ts` senza vincoli (la regola "core non importa una feature" vale solo per `src/core/**`). Risultato: nessun nuovo extension point Hook/Filter introdotto in questa fase — i 5 model si sono spostati in `core/chat/models/` con lo stesso pattern meccanico di 7.1-7.3.

Due aggiunte volute esplicitamente dall'utente oltre al semplice move:
- **Split di `MessageService.ts`**: la metà offGame (`sendOffGameMessage`, ramo offGame di `deleteMessage`, `publishModerationEvent`) si è spostata in `features/offGameMessages/services/OffGameMessageService.ts`. Non più necessario per boundary compliance (il file restava in `modules/`, poteva già importare la feature liberamente), voluto comunque per qualità del codice. Scartato `scheduleDelivery` durante lo split: dead code, zero chiamanti confermati in tutto il repo.
- **Fix dell'asimmetria JWT** fra `core/auth/middleware/auth.ts` (richiede `issuer`/`audience`) e `modules/game/middleware/auth.ts` (non li richiedeva, 5 `jwt.verify()` senza opzioni) — segnalata come nota aperta fin dalla Fase 7.1, chiusa qui anche se il file non è entrato in core in questa fase (il fix non dipende da dove vive il file).

**Test E2E ha trovato un secondo bug preesistente slegato dallo spostamento** (dopo i due di Fase 7.2): `MessageBackup.ts` calcolava `retentionUntil` (campo `required`) dentro un hook `pre('save')`, ma Mongoose valida i campi required PRIMA di `pre('save')` (solo `pre('validate')` gira prima) — la validazione falliva sempre, `MessageBackupService.createBackup()` non ha mai funzionato per nessuno dei due `messageContext`, cioè l'eliminazione di un messaggio (onGame o offGame) con creazione di backup non ha mai funzionato in nessuna versione precedente. Corretto su decisione esplicita dell'utente: `pre('validate')` invece di `pre('save')`.

Testato E2E su Docker sul percorso HTTP reale: invio messaggio onGame (`POST /game/chats`, via `modules/game/middleware/auth.ts` — esercita anche il fix JWT) con verifica diretta del mirror `ChatBackup` e della creazione di una `ChatScene` da parte dell'hook fineSessione; `searchChat` (semantic + fallback); invio ed eliminazione di un messaggio offGame tramite il nuovo `OffGameMessageService`, con verifica del `MessageBackup` creato correttamente dopo il fix. Un test è incappato nel bug preesistente già noto e preservato in Fase 6.8 (`OffGameThreadService.findOrCreateThread` fallisce su thread nuovi per lo stesso motivo pre-save/required) — aggirato creando a mano un thread di test, non toccato di nuovo qui. `type-check`/`build`/`lint:boundaries` puliti, `dump-routes` diff zero.

**Fase 7 completa.** `database/models/` contiene solo model mai stati nello scope originario (Skill, SystemConfiguration, GamingSession, CharacterNotes, SkillConfrontation/CombatEncounter, ecc.). Tutti e 4 gli obiettivi (`Character`/`CharacterSession`, `User`/`auth`, `Location`, `Chat`/`OnGame`) sono in `src/core/`. Il branch resta **non mergiato**, per regola permanente confermata dall'utente — merge manuale solo dopo test approfonditi da parte sua.
