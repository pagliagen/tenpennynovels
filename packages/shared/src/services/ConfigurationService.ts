...
export class ConfigurationService {
  ...
  async subscribeToUpdates(): Promise<void> {
    try {
      ...
      await subscriber.subscribe('system_config_updated', 'system_config_cache_invalidated');

      subscriber.on('message', async (channel, message) => {
        try {
          const data = JSON.parse(message);

          if (channel === 'system_config_updated') {
            const { configKey } = data;
            const cacheKey = `${CACHE_PREFIX}${configKey}`;
            await this.redis.del(cacheKey);
            this.logger.info(`Cache invalidated for updated config: ${configKey}`);
          } else if (channel === 'system_config_cache_invalidated') {
          ...
          this.logger.error('Error processing configuration update event:', error);
        }
      });
