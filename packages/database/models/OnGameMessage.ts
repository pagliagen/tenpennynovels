...
export interface IOnGameMessage extends Document {
  ...
  from: ObjectId; // Character ID
  to: ObjectId[]; // Recipient Character IDs
  subject: string;
  content: string;
  ...
  sentFromLocation: ObjectId; // Location where message was sent
  ...
  // Message Chain
  replyTo?: ObjectId; // Reference to original message
